...
def main():
    ...
    try:
        ...
        cur.execute("DELETE FROM missing_trips WHERE trip_start_date < %s", (cutoff_date,))
        counts["missing_trips_deleted"] = cur.rowcount
        ...
        conn.commit()
        print("Housekeeping klar (cutoff %s): %s" % (cutoff_date, counts))
    except Exception as exc:
        conn.rollback()
        error = str(exc)
        print("FEL under housekeeping: %s" % error)
    finally:
        cur.execute(
            """INSERT INTO housekeeping_runs
               (run_at, cutoff_date, delays_deleted, cancellations_deleted, seen_trips_deleted,
                missing_trips_deleted, alerts_deleted, scan_runs_deleted, error)
               VALUES (%(run_at)s, %(cutoff_date)s, %(delays_deleted)s, %(cancellations_deleted)s,
                       %(seen_trips_deleted)s, %(missing_trips_deleted)s, %(alerts_deleted)s,
                       %(scan_runs_deleted)s, %(error)s)""",
            {
                ...
                "missing_trips_deleted": counts.get("missing_trips_deleted"),
